            ...
            for line_result in BufReader::new(file).lines() {
                ...
                let raw_value: serde_json::Value = match serde_json::from_str(&line) {
                    ...
                    Err(e) => {
                        ...
                        row.message_type = "_parse_error".to_string();
                        row.parse_status = Some("invalid_json".to_string());
                ...
                let parsed: CodexLine = match serde_json::from_value(raw_value.clone()) {
                    ...
                    Err(e) => {
                        let message_type = raw_value
                            .get("type")
                            .and_then(|v| v.as_str())
                            .unwrap_or("_parse_error")
                            .to_string();
                        let timestamp = raw_value
                            .get("timestamp")
                            .and_then(|v| v.as_str())
                            .map(String::from);
                        let base = Self::codex_base_row(
                            ...
                            current_model.as_deref(),
                            &source_path,
                            raw_value.get("payload"),
                        );
                        rows.push(Self::codex_preserved_row(
                            base,
                            &message_type,
                            "unsupported_schema",
                            &line,
                ...
                let row = Self::codex_line_to_row(
                    &parsed,
                    session_uuid,
                    &file_name,
                    file_line,
            ...
            "response_item" => {
                ...
                match item.item_type.as_deref() {
                    ...
                    Some(other) => {
                        ...
                        row.message_content = Some(parsed.payload.to_string());
                        row
                    }
